...
<body>
    ...
    <div id="window-csv" class="modal">
        <form id="form-csv" class="modal-container p-6" method="post" action="{{ route('file.csv-export', ['name' => Auth::user()->name]) }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="filter-container-one-column centered distance-top">
                <div class="modal-card-item-division">
                    ...
                    <button onclick="closeModal('window-csv')">Generar CSV</button>
                </div>
            </div>
        </form>
    </div>
