...
@section('title', 'Profile Role - Developers')
@section('content')
    <main>
        <aside-medium>
            <div class="aside-container" name="main_group">
                {{-- <div class="py-12"> --}}
                                        ...
                                        <form id="userProfileForm" class="mt-6 space-y-6">
                                            ...
                                            <div>
                                                ...
                                                <x-text-input id="trade_id" name="trade_id" type="text"
                                                    class="mt-1 block w-full" :value="old('trade_id', $user->trade_id)"
                                                    required autofocus autocomplete="trade_id" disabled />
                                                <x-input-error class="mt-2" :messages="$errors->get('trade_id')" />
                                            </div>

                                            <div>
                                                <x-input-label for="role_type" :value="__('Role Type')" />
                                                <x-select name="role_type" :options="$profile_list"
        ...
        <section-medium class="main-section" id="main_group" name="main_group">
            <div class="table-row" id="developer-container">
                <div class="modal-card-item-division">
                    <div class="list-text-large"><strong>Email</strong>
                    </div>
                    <div class="list-text-medium"><strong>Developer</strong>
                    </div>
                </div>
                @foreach ($users as $key => $userdev)
                    <div class="modal-card-item-division">
                        <div class="list-text-large">{{ $userdev['email'] }}
                        </div>
                        <div class="list-text-medium">{{ $userdev['name'] }}
                        </div>
                    </div>
                @endforeach
            </div>
